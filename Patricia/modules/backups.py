...
from Patricia import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER
...
def export_data(update, context):
    ...
    f = open("Patricia{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Patricia{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Patricia-Backup` was specially made for notes."
        .format(chat.title, chat_id, tgl),
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove("Patricia{}.backup".format(chat_id))  # Cleaning file
